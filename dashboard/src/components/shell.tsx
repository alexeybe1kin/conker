...
function Navigation({ close }: { close?: () => void }) {
  const { decisions, signOut } = usePreview()
  const [menu, setMenu] = useState(false)
  const count = ["coach", "revision", "cleanup"].filter(id => !decisions[id]).length
  return <div className="navigation"><Link className="wordmark" to="/" onClick={close}><Mark small />Conker</Link>
    ...
    <div className="sidebar-foot"><p className="sidebar-note">A little more room<br />for everything else.</p>
      <div className="owner-wrap">{menu && <div className="owner-menu"><p><Palette aria-hidden="true" />Appearance</p><ThemeToggle /><Button variant="ghost" onClick={() => { signOut(); close?.() }}><LogOut aria-hidden="true" />Sign out of preview</Button></div>}
        <Button variant="ghost" className="owner-button" onClick={() => setMenu(!menu)} aria-expanded={menu} aria-label="Owner menu"><span className="avatar">{ownerFixture.initials}</span><span className="owner-text"><strong>{ownerFixture.name}</strong><small>Your own space</small></span><ChevronUp aria-hidden="true" /></Button>
      </div>
    </div></div>
}
