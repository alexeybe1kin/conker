...
function ApprovalCard({ item, expanded }: { item: Approval; expanded: boolean }) {
  ...
  return <Card className="approval-card" id={item.id}>
    ...
    </CardContent><CardFooter className="approval-footer"><div className="decision-buttons"><Button variant="outline" disabled={!actionable} onClick={() => decide(item.id, "denied")}>Deny</Button><Button disabled={!actionable} onClick={() => decide(item.id, "approved")}>Approve once<ArrowRight /></Button></div><Link className="policy-link" to={`/agents/${item.agent.toLowerCase()}?review=grants`}>Raise this agent’s autonomy instead →</Link><p className="fine-print"><Clock3 />Decide by {item.decision} <span>·</span> Spend window {item.spendSeconds / 60} min once approved</p>{decided === "denied" && reason && <p className="fine-print">Your reason: {reason}</p>}</CardFooter></>}
