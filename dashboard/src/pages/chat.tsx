...
export function Chat() {
  ...
  const { sessions, addSession, decisions } = usePreview()
  ...
  useEffect(() => { setMessages([]); setDraft(""); setStreaming(false); setReplied(false) }, [sessionId])
  useEffect(() => {
    if (!streaming) return
  ...
  <div className="thread"><div className="thread-heading"><h2>{session.title}</h2><Button size="sm" variant="ghost" onClick={() => create(true)}><GitFork />Fork</Button></div>
    ...
    {session.mode === "plan" && <>
      ...
      <article className="message assistant-message"><div className="message-label"><Mark small />Conker</div>
    ...
    {stream > 0 && <article className="message assistant-message"><div className="message-label"><Mark small />Conker · scripted preview</div><p>{demoReply.slice(0, stream)}{streaming && <span className="stream-cursor" aria-hidden="true" />}</p></article>}
    <div ref={bottom} />
